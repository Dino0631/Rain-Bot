...
#         return
    ...
    if heroku==True:
        ...
        if message.content.startswith(prefix):
            ...
            return
    if message.server == dukeserver and not dataIO.load_json(NOB_JSON)['nob'] and not (a == '' or a == None or '`nob`' in a):
        b = ''
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    @bot.event
    async def on_ready():
        bot.on_message = on_message2 #add duke meme thing
